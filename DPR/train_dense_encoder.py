...
from dpr.models.biencoder import BiEncoder, BiEncoderNllLoss, BiEncoderBatch, DPOLoss, BiEncoder_list_ranking_loss
...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        if IF_PREFER_LOSS:
            _, ref_model, _ = init_biencoder_components(
                cfg.encoder.encoder_model_type, cfg, inference_only=True
            )
            ref_model, _ = setup_for_distributed_mode(
                ref_model,
                None,
                cfg.device,
                cfg.n_gpu,
            ...
            self.ref_biencoder = ref_model.eval()
            if os.environ.get('PT_MODEL', False) and saved_state:
                model_to_load = get_model_obj(self.ref_biencoder)
        ...
        self.cfg = cfg
        self.ds_cfg = BiencoderDatasetsCfg(cfg) # datasets
    ...
    def run_train(self):
        ...
        for epoch in range(self.start_epoch, int(cfg.train.num_train_epochs)):
            ...
            if os.environ.get('PT_MODEL', False) and IF_PREFER_LOSS==6:
                break
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            biencoder_inputs = BiEncoder.create_biencoder_input2(
                samples_batch,
                self.tensorizer,
                True,
                num_hard_negatives,
            ...
            rep_positions = ds_cfg.selector.get_positions(
                biencoder_inputs.question_ids, self.tensorizer
            )
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            ctxs_ids = biencoder_inputs.context_ids
            ctxs_segments = biencoder_inputs.ctx_segments
            bsz = ctxs_ids.size(0)
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_ids, q_segments = (
                    (biencoder_inputs.question_ids, biencoder_inputs.question_segments)
                    if j == 0
                    else (None, None)
                )
            ...
            batch_positive_idxs = biencoder_inputs.is_positive
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            encoder_type = ds_cfg.encoder_type
            # do not shuffle for udr rank loss ablation
            shuffle_positives = ds_cfg.shuffle_positives and IF_PREFER_LOSS != -1
            ...
            if self.loss_type == 'dpr':
                biencoder_batch = BiEncoder.create_biencoder_input2(
                    ...
                    num_hard_negatives,
                    num_other_negatives,
                    shuffle=(True and IF_PREFER_LOSS != -1), # do not shuffle for udr rank loss ablation
            ...
            if i % log_result_step == 0:
                ...
                if self.loss_type == 'dpr':
                    logger.info(
                        "Epoch: %d: Step: %d/%d, loss=%f, lr=%f, pf=%d, r=%f, beta=%f, ckpt=%s, drej=%d",
                        epoch,
                        data_iteration,
                        epoch_batches,
                        loss.item(),
...
def _do_biencoder_fwd_pass(
    ...
    inputs = BiEncoderBatch(**move_to_device(inputs._asdict(), cfg.device))

    q_attn_mask = tensorizer.get_attn_mask(inputs.question_ids)
    ctx_attn_mask = tensorizer.get_attn_mask(inputs.context_ids)

    if model.training:
        model_out = model(
            inputs.question_ids,
            inputs.question_segments,
            q_attn_mask,
            inputs.context_ids,
            inputs.ctx_segments,
            ctx_attn_mask,
    ...
    if ref_model is not None:
        if model.training:
            ref_q_vector, ref_ctx_vectors = ref_model(
                inputs.question_ids,
                inputs.question_segments,
                q_attn_mask,
                inputs.context_ids,
        ...
        local_q_vector = torch.cat([local_q_vector,ref_q_vector],dim=-1)
        local_ctx_vectors = torch.cat([local_ctx_vectors,ref_ctx_vectors],dim=-1)



    if IF_PREFER_LOSS == 0:
        loss_function = BiEncoderNllLoss()
    elif IF_PREFER_LOSS > 0:
        loss_function = DPOLoss(beta=PEFT_BETA,div_rej=DIV_REJECT)
    elif IF_PREFER_LOSS == -1:
        loss_function = BiEncoder_list_ranking_loss()
    else:
        raise NotImplementedError(f"Not Implemented for IF_PREFER_LOSS={IF_PREFER_LOSS}")
...
def main(cfg: DictConfig):
    ...
    cfg.train.batch_size = cfg.train.batch_size if IF_PREFER_LOSS<1 else cfg.train.batch_size*8
    cfg.model_file = os.environ.get('PT_MODEL', cfg.model_file)
    if os.environ.get('PT_MODEL', False):
    ...
    if os.environ.get('PT_MODEL', False):
        version_info = f"[{Path(cfg.model_file).parent.stem}]"+"+"+version_info
    dst_dir = Path(cfg.output_dir).with_name("saves")/(f"{version_info}").replace(".","d")

    trainer = BiEncoderTrainer(cfg)
    if cfg.train_datasets and len(cfg.train_datasets) > 0:
        trainer.run_train()
        dst_fp = dst_dir/Path(trainer.best_cp_name).name
        dst_dir.mkdir(exist_ok=True,parents=True)
        for old_fp in dst_dir.iterdir():
            os.remove(str(old_fp))
        assert not any(dst_dir.iterdir())
        shutil.copy2(trainer.best_cp_name,str(dst_fp))
